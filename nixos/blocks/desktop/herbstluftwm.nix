...
in
{
  ...
  config = mkIf cfg.enable {
    blocks.desktop = {
      windowManager.name = "herbstluftwm";
      xserver.enable = true;
      xserver.wmCommand = '' exec ${package}/bin/herbstluftwm --locked'';
    };
    
    #some rofi scripts
    blocks.programs.rofi.scripts = {
      tags = {
        scriptEntry = ''
          for tag in $(${hc} complete 1 use); do
            list_entry "Go to tag $tag" "${hc} use $tag"
          done
        '';
      };
      
      windowMenu = {
        entry.fullscreen = "${hc} fullscreen toggle";
        entry.floating = "${hc} set_attr clients.focus.floating toggle";
      
        scriptEntry = ''
          for tag in $(${hc} complete 1 move); do
            list_entry "Move window to tag $tag" "${hc} move $tag"
          done
        '';
      };
    };
