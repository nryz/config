def create_left_prompt [with_git: bool = false] {
    ...
    let path_segment = $"(ansi $base0D)($dir)"
    ...
    if $with_git {
        ...
        let git_branch_segment = $"(ansi $base03)($git_branch_name_str)"
        let git_status_segment = $"(ansi $base0E)($git_status_str)"
        let git_commit_segment = $"(ansi $base0D)[($git_ahead_behind_str)]"
...
let-env PROMPT_INDICATOR = {|| $"(ansi $base0E)> " }
let-env PROMPT_INDICATOR_VI_INSERT = {|| $"(ansi $base0E)> " }
let-env PROMPT_INDICATOR_VI_NORMAL = {|| $"(ansi $base0E)> " }
